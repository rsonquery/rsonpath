...
def plot_time(df: pd.DataFrame, save_path: str, column_suffix: str, title: str, ylabel: str, per_key: bool) -> None:
...
def plot_build_time(df: pd.DataFrame, save_path: str) -> None:
    plot_time(
        ...
        title='Compare different lookup table build times',
        ylabel='Build time in seconds',
        per_key=False,
        
    )
    
def plot_query_time(df: pd.DataFrame, save_path: str) -> None:
    plot_time(
        df=df,
        save_path=save_path,
        column_suffix='_query_time',
        title='Time of each lut for querying every key once.',
...
def plot_build_time_per_key(df: pd.DataFrame, save_path: str) -> None:
    plot_time(
        ...
        save_path=save_path,
        column_suffix='_build_time',
        title='Compare different lookup table build times per key',
        ylabel='Build time in seconds',
        per_key=True,
        
    )
    
def plot_query_time_per_key(df: pd.DataFrame, save_path: str) -> None:
    plot_time(
        df=df,
        save_path=save_path,
        column_suffix='_query_time',
        title='Time of each lut for querying every key once divided by num of keys.',
        ylabel='Average query time per key',
        per_key=True,
    )
...
if __name__ == "__main__":
    ...
    plot_query_time(df, os.path.join(directory, f"{file_base_name}_query_time.png"))
    plot_build_time_per_key(df, os.path.join(directory, f"{file_base_name}_build_time_per_key.png"))
    plot_query_time_per_key(df, os.path.join(directory, f"{file_base_name}_query_time_per_key.png"))
