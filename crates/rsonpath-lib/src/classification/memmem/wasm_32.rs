...
impl MemmemImpl for Constructor {
    type Classifier<'i, 'b, 'r, I, R>
        = WasmMemmemClassifier32<'i, 'b, 'r, I, R>
    where
        I: Input + 'i,
...
pub(crate) struct WasmMemmemClassifier32<'i, 'b, 'r, I, R>
where
    I: Input,
...
impl<'i, 'b, 'r, I, R> WasmMemmemClassifier32<'i, 'b, 'r, I, R>
where
    I: Input,
...
impl<'i, 'b, 'r, I, R> Memmem<'i, 'b, 'r, I, SIZE> for WasmMemmemClassifier32<'i, 'b, 'r, I, R>
where
    I: Input,
    R: InputRecorder<I::Block<'i, SIZE>>,
    'i: 'r,
{
    ...
    fn find_label(
        ...
        // SAFETY: wasm SIMD invariant handled in vector_128 with #[target_feature(enable="simd128")].
        unsafe { self.find_label_wasm(label, next_block_offset) }
    }
}
