...
mod avx2 {
    ...
    impl<'a> Avx2Classifier<'a> {
        ...
        fn next_block(&mut self) -> bool {
            ...
            while self.current_block_is_spent() {
                ...
                self.block = unsafe {
                    Some(
                        self.classifier
                            .classify(self.bytes, &self.bytes[BLOCK_SIZE..]),
                    )
                };
                self.bytes = &self.bytes[2 * BLOCK_SIZE..];
                self.offset += 2 * BLOCK_SIZE;
            }
        ...
        fn current_block_is_spent(&self) -> bool {
            self.block.as_ref().map_or(true, |x| Empty::is_empty(x))
        }
    }
    ...
    impl BlockAvx2Classifier {
        const LOWER_NIBBLE_MASK_ARRAY: [u8; 32] = [
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x00, 0x02,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02,
            0x00, 0x02, 0x00, 0x00,
        ];
        const UPPER_NIBBLE_MASK_ARRAY: [u8; 32] = [
            0xFF, 0xFF, 0xFF, 0x01, 0xFF, 0x02, 0xFF, 0x02, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
            0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x01, 0xFF, 0x02, 0xFF, 0x02, 0xFF, 0xFF, 0xFF, 0xFF,
            0xFF, 0xFF, 0xFF, 0xFF,
        ];
        ...
        #[target_feature(enable = "avx2")]
        #[inline]
        unsafe fn classify<'a>(&mut self, block1: &'a [u8], block2: &[u8]) -> StructuralsBlock<'a> {
            ...
            let slashes =
                (classification1.slashes as u64) | ((classification2.slashes as u64) << 32);
            ...
            let cumulative_xor =
                _mm_clmulepi64_si128::<0>(nonescaped_quotes_vector, self.all_ones128);
            ...
            StructuralsBlock::new(block1, nonquoted_structural)
        }
