//! Source code generation for `find_byte_sequenceN` functions with SSE2 support.
...
fn get_find_byte_sequence_dispatch_source() -> TokenStream {
    ...
    quote! {
        ...
        /// support SSE2 this will fallback to [`nosimd::find_byte_sequence`].
        /// # Examples
        /// ```
        /// # use simd_benchmarks::sequences::sse2::find_byte_sequence;
        /// # use aligners::{alignment::TwoTo, AlignedBytes};
        /// let bytes: AlignedBytes<TwoTo<4>> = "abcdefgh".as_bytes().into();
        ...
        /// ```
        #[inline]
        pub fn find_byte_sequence(sequence: &[u8], bytes: &AlignedSlice<TwoTo<4>>) -> Option<usize> {
            #[cfg(target_feature = "sse2")]
            {
                if bytes.len() < BYTES_IN_SSE2_REGISTER * 2 {
                    return nosimd::find_byte_sequence(sequence, bytes);
                }

                match sequence.len() {
                    0 => panic!("Cannot look for an empty sequence."),
                    1 => crate::find_byte::find_byte_size128(sequence[0], bytes),
                    #(#match_body,)*
...
fn get_find_byte_sequence_source(n: usize) -> TokenStream {
    ...
    let declarations = (0..n).map(|i| {
        ...
        quote! {
            let #mask_ident = _mm_set1_epi8(sequence[#i] as i8);
            let #cmp_mask_first_block_vector_ident = _mm_cmpeq_epi8(first_block, #mask_ident);
            let mut #cmp_mask_first_block_ident = _mm_movemask_epi8(#cmp_mask_first_block_vector_ident) as u16;
        }
    });
    ...
    let mask_calculations = (0..n).map(|i| {
        ...
        let mask_computation = if i > 0 {
            quote! {
                let #cmp_mask_ident = ((#cmp_mask_first_block_ident as u32) | ((#cmp_mask_next_block_ident as u32) << 16)) >> #i;
            }
        } else {
            quote! {
                let #cmp_mask_ident = (#cmp_mask_first_block_ident as u32) | ((#cmp_mask_next_block_ident as u32) << 16);
            }
        };

        quote! {
            let #cmp_mask_next_block_vector_ident = _mm_cmpeq_epi8(next_block, #mask_ident);
            let #cmp_mask_next_block_ident = _mm_movemask_epi8(#cmp_mask_next_block_vector_ident) as u16;
            #mask_computation
        }
    });
    ...
    quote! {
        ...
        #[allow(dead_code)]
        #[inline]
        unsafe fn #ident(sequence: &[u8], bytes: &AlignedSlice<TwoTo<4>>) -> Option<usize> {
            debug_assert!(sequence.len() == #n);

            let iter = bytes.iter_blocks().enumerate();
            let first_block = _mm_load_si128(bytes.as_ptr() as *const __m128i);
            #(#declarations)*

            for (i, block) in iter {
                let ptr = block.as_ptr() as *const __m128i;
                let next_block = _mm_load_si128(ptr.offset(1));

                #(#mask_calculations)*
                #(#cmp_and_tree_instructions)*

                if #root_cmp_node_ident != 0 {
                    return Some(i * 16 + (#root_cmp_node_ident.trailing_zeros() as usize));
                }
...
fn get_find_long_byte_sequence_source() -> TokenStream {
    quote! {
        ...
        #[allow(dead_code)]
        #[inline]
        unsafe fn find_long_byte_sequence(sequence : &[u8], bytes: &AlignedSlice<TwoTo<4>>) -> Option<usize> {
            let mut bytes = bytes;
            let mut i = 0;

            while bytes.len() >= sequence.len() {
                let heuristic_match = find_byte_sequence16(&sequence[..16], bytes);

                if let Some(j) = heuristic_match {
                    if (&bytes[j + 16..]).starts_with(&sequence[16..]) {
                        return Some(i + j);
                    }
                    let offset = j / 16 + 1;
