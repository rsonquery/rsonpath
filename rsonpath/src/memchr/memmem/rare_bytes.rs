...
impl RareBytes {
    ...
    /// needle.
    pub(crate) fn for_needle(needle: &[u8]) -> RareBytes {
        if needle.len() <= 1 {
            panic!("needle cannot be a single character");
        }

        let needle = if needle.len() > 32 {
            &needle[..64]
        } else {
            needle
        };
...
fn rank(b: u8) -> usize {
    super::byte_frequencies::BYTE_FREQUENCIES[b as usize] as usize
}
